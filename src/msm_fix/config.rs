...
pub struct FixMSMGate<F: PrimeField + Ord, App: CurveAffine<Base = F>> {
    ...
    pub(crate) x_table: TableColumn,
    pub(crate) y_table: TableColumn,
    pub(crate) address_table: TableColumn,
    ...
    pub(crate) memory: BTreeMap<F, (F, F)>,
    pub(crate) window: usize,
    pub(crate) constants: BTreeMap<F, AssignedValue<F>>,
    pub(crate) correction: App,
...
impl<F: PrimeField + Ord, App: CurveAffine<Base = F>> FixMSMGate<F, App> {
    pub fn configure(
        ...
        address_table: TableColumn,
        x_table: TableColumn,
        y_table: TableColumn,
        constant: Column<Fixed>,
        window: usize,
        memory: BTreeMap<F, (F, F)>,
        correction: App,
    ) -> Self {
        ...
        meta.lookup("windowed point table", |meta| {
            let s_query = meta.query_selector(s_query);
            let query_base = meta.query_fixed(constant, Rotation::cur());
            let query_offset = meta.query_advice(a0, Rotation::cur());
            let query_x = meta.query_advice(a1, Rotation::cur());
            let query_y = meta.query_advice(a2, Rotation::cur());
            vec![
                (e!(s_query) * (query_base + query_offset), address_table),
                (e!(s_query) * query_x, x_table),
                (e!(s_query) * query_y, y_table),
            ]
        });
